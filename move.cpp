...
void Board::push(Moves move){
    ...
    //En passant flag
        ...
        move_piece(move.from_square, move.to_square, turn, move.piece);
        remove_piece(move.to_square + pawn_shift, opp_col, PAWN_I);
    }
...
void Board::pop(){
    ...
    //Kingside Castle flag
        ...
        move_piece(pos.move.to_square, pos.move.from_square, turn, pos.move.piece);
        move_piece(pos.move.from_square+1, pos.move.to_square+1, turn, ROOK_I);
    }
    //Queenside Castle flag
    else if (pos.move.flag == QS_F) {
        move_piece(pos.move.to_square, pos.move.from_square, turn, pos.move.piece);
        move_piece(pos.move.from_square-1, pos.move.to_square-2, turn, ROOK_I);
    }
    //Promote (without capture) flag
    else if (pos.move.flag == PROMOTE_F) {
        add_piece(pos.move.from_square, turn, pos.move.piece);
        remove_piece(pos.move.to_square, turn, pos.move.promoted);
    }
    //Promote (with capture) flag
    else if (pos.move.flag == PROMOTE_F) {
        add_piece(pos.move.to_square, opp_col, pos.move.captured);
        add_piece(pos.move.from_square, turn, pos.move.piece);
        remove_piece(pos.move.to_square, turn, pos.move.promoted);
    }
    //En passant flag
        ...
        turn == WHITE ? pawn_shift = 8 : pawn_shift = -8;
        move_piece(pos.move.to_square, pos.move.from_square, turn, pos.move.piece);
        add_piece(pos.move.to_square + pawn_shift, opp_col, PAWN_I);
    }
    turn = opp_col;
}